...
def route_order(ch, method, properties, body):
    ...
    for index, item in enumerate(items):
        ...
        if routing_key is None:
            # Unknown item type: don't let it silently vanish. Log it and
            # fall back to the physical queue as a defensible default,
            # since physical fulfillment is the most conservative path
            # for something we don't otherwise recognize.
            print(f"[Router] Unknown item type '{item_type}' for order {order_id}, "
                  f"defaulting to orders.physical")
