...
const orderId =
line.slice(...ORDER_LAYOUT.orderId).trim();

const customerId =
line.slice(...ORDER_LAYOUT.customerId).trim();

const customerName =
line.slice(...ORDER_LAYOUT.customerName).trim();

const orderDate =
line.slice(...ORDER_LAYOUT.orderDate).trim();

const amount =
line.slice(...ORDER_LAYOUT.amount).trim();

const currency =
line.slice(...ORDER_LAYOUT.currency).trim();
...
const customerCsv = readFileSync(
...
// Find customers with no accepted orders
const orderedCustomerIds = new Set(
...
if (!orderedCustomerIds.has(customerId)) {
